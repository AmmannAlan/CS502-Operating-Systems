...
void test2(void) {
	...
	if ((DataRead->int_data[0] != DataWritten->int_data[0])
		...
		printf("Written:  %d,  %d,  %d,  %d\n", DataWritten->char_data[0],
			DataWritten->char_data[1], DataWritten->char_data[2],
			DataWritten->char_data[3]);
		printf("Read:     %d,  %d,  %d,  %d\n", DataRead->char_data[0],
			DataRead->char_data[1], DataRead->char_data[2],
			DataRead->char_data[3]);
...
void test3(void) {
	...
	printf("Error Returned: %ld", ErrorReturned);
	

	// Create two processes with same name - 1st succeeds, 2nd fails
	// Then terminate the process that has been created
	CREATE_PROCESS("two_the_same", testX, LEGAL_PRIORITY, &ProcessID2, &ErrorReturned);
...
void test5(void) {
	...
	printf("\n********************************************* \n TEST D has ended. Test 5 Continues. \n ************************************* \n");
	GET_TIME_OF_DAY(&CurrentTime);
...
void testX(void) {
	...
	for (Iterations = 0; Iterations < NUMBER_OF_TESTX_ITERATIONS;
		...
		printf("Test X ending time: %d,\n", EndingTime);
		printf("Test X Random Seed: %d,\n", RandomSeed);
...
/**************************************************************************
 ...
 In studentConfiguration.h, there's a variable  DO_MEMORY_DEBUG. Switching it to
...
// This is an array containing the memory address we are accessing
	INT32 test_data[TEST_DATA_SIZE ] = { 0, 4, PGSIZE - 2, PGSIZE, 3 * PGSIZE - 2, (NUMBER_VIRTUAL_PAGES - 1) * PGSIZE, NUMBER_VIRTUAL_PAGES * PGSIZE - 2 };
...
void test24(void) {
	...
	for (Index = 0; Index < LOOP_COUNT; Index++) // Bugfix Rel 4.03  12/1/2013
		mtr->page_touched[Index] = 0; //initialize the array
	...
	for (Loops = 0; Loops < LOOP_COUNT; Loops++) {
		...
		if (Loops % DISPLAY_GRANULARITY24 == 0)  //0,200,400,600,800,...,3600,3800
	          printf("PID= %ld  address= %4ld   written= %4d   read= %4d\n",
				OurProcessID, MemoryAddress, DataWritten, DataRead);

		if (DataRead != DataWritten) {
			printf("\n ERROR HAS OCCURRED: READ NOT SAME AS WRITE.\n");
			printf("PID= %ld  address= %4ld   written= %4d   read= %4d\n",
					OurProcessID, MemoryAddress, DataWritten, DataRead);
